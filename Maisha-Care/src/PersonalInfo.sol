...
contract PersonalInfo {
    ...
    event InfoUpdateInitiated(address indexed patient, address indexed doctor, bytes32 updateId);
    ...
    function initiateInfoUpdate(address _patient, string memory _dataHash) public {
    ...
    function updateInfo(bytes32 _updateId) public {
        ...
        personalInfo[patient] = Info(bytes32ToString(dataHash), block.timestamp);
        emit InfoUpdated(patient, block.timestamp);
    }

    function getInfo(address _patient) public view returns (string memory, uint256) {
        require(
